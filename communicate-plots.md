...
ggplot(millas, aes(motor, autopista)) +
  ...
  labs(title = "La eficiencia en el uso de combustible disminuye con el tamaño del motor")
```
...
<img src="communicate-plots_files/figure-html/just-1.png" alt="Las nueve combinaciones posibles con `hjust` y `vjust`." width="60%" />
...
compacto <- millas %>% filter(clase == "compact")
